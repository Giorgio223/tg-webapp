...
async function ensureState(now: number) {
  ...
  // если по какой-то причине время "в будущем" — исправляем
  if (state.phaseStartedAt > now + 2000) state.phaseStartedAt = now;

  // прокручиваем фазы, если сервер долго не дергали
  for (let i = 0; i < 200; i++) {
  ...
  const history = histRaw
    .map((x) => {
      try {
        return JSON.parse(x) as { t: number; v: number };
      } catch {
        return null;
      }
    })
